...
- **compact 모드 = 단일 테이블** (2026-05-01 추가): 같은 마을 fallback 5건은 어차피 변전소/주변압기/선로가 거의 동일해 그룹 분리하면 같은 모양 표 2~3개 반복 = 노이즈. compact=true 일 때 그룹화 로직 자체를 우회하고 `CompactTable` 분리 컴포넌트로 단일 테이블 렌더.
- **HTTP 캐시**: `private, max-age=600` (10분).
- **village_empty 플래그** (2026-05-01 추가): fallback RPC 가 빈 배열 반환 = 마을(bjd_code) 전체에 한전 데이터 0건. 추가 쿼리 0 — 같은 RPC 결과로 판정. UI 에서 "이 마을 전체에 한전 용량 정보가 없습니다 → 한전 직접 확인 필요" 별도 안내. 강원 고성군 88개 리 중 85개(97%)가 이 케이스.
